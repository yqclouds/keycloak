 ...
 * @version $Revision: 1 $
 */
public class RealmFacadeImpl implements RealmFacade, ApplicationEventPublisherAware {
    @Autowired
    ...
    @Override
    public RealmModel createRealm(RealmRepresentation rep) {
        RealmModel result = createRealm(rep, false);

        grantPermissionsToRealmCreator(result);
    ...
    public List<RealmRepresentation> getRealms() {
        List<RealmRepresentation> results = new ArrayList<>();

        List<RealmModel> realms = realmProvider.getRealms();
        for (RealmModel realm : realms) {
            addRealmRep(results, realm);
        }
     ...
     */
    public RealmModel createRealm(RealmRepresentation rep, boolean skipUserDependent) {
        String id = rep.getId();
        if (id == null) {
